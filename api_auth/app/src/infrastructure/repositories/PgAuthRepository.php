...
class PgAuthRepository implements AuthRepositoryInterface{
    ...
    {
        try {
            $query = 'INSERT INTO users (id, email, password, pseudo, role) VALUES (:id, :email, :password, :pseudo, :role) RETURNING id;';
            ...
            return new User($userId, $user->email, $user->password, $user->role, $user->pseudo);
        } catch (\PDOException $e) {
    ...
    try {
        ...
        return new User($user['id'], $user['email'], $user['password'], $user['role'], $user['pseudo']);
    } catch (\PDOException $e) {
